...
**Available now.** Workspaces can connect a Jira Cloud site over Atlassian OAuth 2.0, choose a default project, create Jira issues from Woes support issues (by hand or from an automation), and keep status in sync in both directions between the support issue and the linked ticket.
...
- **Status sync (Jira → Woes).** A change to the linked Jira issue flows back into Woes. When the ticket moves to a new workflow category (To Do / In Progress / Done), Woes updates the linked support issue's status to match. Woes registers a per-workspace Jira webhook at connect time and refreshes it automatically before it expires.
...
- **Disconnect** at any time from the Jira card. Disconnecting deletes the stored connection and tokens and removes the Jira webhook.
...
2. Add the **Jira platform REST API** and these scopes: `read:jira-work`, `write:jira-work`, `read:jira-user`, `offline_access`, `manage:jira-project`, `manage:jira-webhook` (`offline_access` is required so Woes receives refresh tokens; `manage:jira-webhook` is required for Jira → Woes status sync).
...
Woes stores **no Atlassian personal data** — no account IDs, names, emails, avatars, reporters, assignees, or commenter identities. Inbound sync carries the Jira issue key and its status category only. The inbound webhook is authenticated by a high-entropy token that Woes stores as a salted hash, never in plaintext.
