...
- `POST /api/widget/updates/read`

The SDK never uses REST API keys, Supabase credentials, workspace secrets, operator debug traces, or provider/model internals. Conversation history still depends on the per-widget-key `conversationSecret`, and update read state should use an opaque verified reader key rather than raw secrets or private account data.
