...
<Note>
  The widget uses a public widget key such as `trovw_...` or `trovw_agent_...`. Do not use a `woesk_` REST API key in browser code.
</Note>
...
<textarea className="woes-agent-prompt-source" data-woes-agent-prompt="mintlify-widget" readOnly>
  ...
  * Use a public widget key only. Never use or ask for a Woes REST API key, Supabase key, workspace secret, or identity verification secret.
  ...
  8. After the install file is in place and validated, ask me for the Woes public widget key. It should look like trovw\_... or trovw\_agent\_....
...
<Tip>
  The Woes docs site uses this same pattern with its own public widget key: `trovw_agent_07f3655b89074d4cb7763725fcfafc64`.
</Tip>
