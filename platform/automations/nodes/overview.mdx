...
The builder intentionally exposes configurable primitives instead of many duplicate preset nodes. For example, choose **Set priority** and then pick `urgent`, `high`, `medium`, or `low`; choose **Snooze issue** and then pick the duration.
...
| Inbox | Add internal note, Add tag, Archive issue, Set priority, Set status, Snooze issue |
...
A single runtime event can satisfy more than one trigger. For example, a new live chat conversation can match both **Issue created** and **New live chat conversation**, so generic issue routing and live-chat-specific routing can run in the same pass.
