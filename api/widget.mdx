...
The config route returns presentation fields only: colors, logo URLs, welcome copy, quick links, position, radius, theme, identity-required state, enabled surveys, published updates settings, business-hours fields, and enabled state. It does not return workspace settings, secrets, operator data, or API key material.
