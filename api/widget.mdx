...
| `setTheme` | `"dark"`, `"light"`, `"system"`, or `"auto"` | Sets the widget theme behavior. |
...
For verified installs, pass either an HMAC email proof or a JWT minted by your backend. Verified identity should be fetched from an authenticated, no-store backend endpoint and retried after login/session load, widget load, focus, and visibility changes. See [Widget Identity](/security/widget-identity) for the full security model.
...
| `/api/widget/identity` | HMAC/JWT identity proof verification. |
...
| `/api/widget/feedback` | Customer-safe message feedback state for a verified widget conversation. |
| `/api/widget/csat` | Compatibility route for CSAT and NPS widget survey responses. |
| `/api/widget/updates` | Published widget updates. |
| `/api/widget/updates/read` | Verified-reader update read state. |
...
Use [Widget Routes](/api/widget-routes) for route-by-route request requirements and boundary details.
