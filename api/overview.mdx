...
<Warning>
  Use a disposable test key in the docs playground. If you need scoped test keys,
  create them through backend or administrative tooling until the Settings scope
  picker is available. Do not paste a production integration key. Playground
  requests run against `https://woes.dev` and may transit the docs runtime when
  proxying is enabled. For production keys, run `curl` or SDK calls from your
  own backend and revoke playground test keys after use.
</Warning>
