...
| `/api/mcp` | Remote MCP JSON-RPC endpoint. Discovery is public-safe; tool calls require `Authorization: Bearer <woesk_...>`. |
...
| `/api/client-portal/issues/{issueId}/messages` | Client Portal issue message creation. |
| `/api/client-portal/members` | Client Portal member list and invite flows. |
...
Some Woes features receive server-to-server events from connected providers.
These managed event routes do not accept workspace API keys and are not direct
customer API surfaces.

| Event family | Purpose |
| --- | --- |
| Billing events | Keep subscription status and included limits in sync. |
| Email delivery events | Receive inbound email and delivery updates for configured email channels. |
| Channel events | Receive approved activity from connected support channels such as Discord or Slack. |
...
<Note>
  ...
  tokens, public widget keys, conversation secrets, or platform-admin access
  depending on the route family.
</Note>
